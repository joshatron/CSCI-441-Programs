...
void Shape::updateBrickLocsRect(double brickWidth, double brickHeight, double brickDepth, double spacing, double scaleX, double scaleY, double scaleZ, int wallDepth)
{
    ...
    double startX = (-1 * (width / 2.) * (brickWidth + spacing)) + (spacing / 2) + (brickWidth / 2);
    double startZ = (-1 * (depth / 2.) * (brickWidth + spacing)) + (spacing / 2) + (brickWidth / 2);

    bool even = starting;
    float yLoc = scaleY * startY;
    for(int k = 0; k < (int)(scaleY * height); k++)
    {
        float loc = startX + brickDepth;
        int widthNum = width;
        if(even)
        {
            loc += brickDepth + spacing;
            widthNum--;
        }

        for(int a = 0; a < widthNum; a++)
        {
            brickLocs.push_back(translate(mat4(1.f), vec3(loc, yLoc, startZ)) * rotate(mat4(1.f), (float)angle, vec3(0,1,0)));
            brickLocs.push_back(translate(mat4(1.f), vec3(loc, yLoc, startZ * -1)) * rotate(mat4(1.f), (float)angle, vec3(0,1,0)));
            loc += brickWidth + spacing / 2;
        }

        loc = startZ + brickDepth;
        int depthNum = depth;
        if(!even)
        {
            loc += brickDepth + spacing;
        ...
        {
            brickLocs.push_back(translate(mat4(1.f), vec3(startX, yLoc, loc)) * rotate(mat4(1.f), (float)(90. + angle), vec3(0,1,0)));
            brickLocs.push_back(translate(mat4(1.f), vec3(startX * -1, yLoc, loc)) * rotate(mat4(1.f), (float)(90. + angle), vec3(0,1,0)));
            loc += brickWidth + spacing / 2;
        }
